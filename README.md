...
Playwright 기반 웹서비스 점검 도구입니다. 패키지를 설치한 뒤 `npx site-check-pro run`으로 로컬 콘솔을 띄우고, 콘솔에서 비로그인/로그인 프로필을 선택해 사이트를 점검합니다.
...
- **페이지 진입**: 대상 경로에 접속하고 문서 응답 상태가 실패 기준 이상인지 확인합니다.
- **렌더링 확인**: `body`에 실제 렌더링된 요소와 최소 텍스트가 있는지 확인해 빈 화면이나 불완전한 렌더링을 감지합니다.
- **콘솔 오류**: 브라우저 콘솔 오류와 런타임 예외를 수집해 실패로 기록합니다.
- **네트워크 요청**: 실패한 요청과 HTTP 실패 응답을 확인합니다.
- **새로고침**: 페이지 새로고침 후 URL 변경이나 실패 응답이 발생하는지 확인합니다.
- **뒤로가기/앞으로가기**: 브라우저 히스토리 이동 후 기대한 URL로 복원되는지 확인합니다.
- **링크 탐색**: 페이지 안의 링크를 수집해 설정된 범위 안에서 다음 경로를 계속 점검합니다.
- **API 상태 확인**: 설정에 등록한 API를 호출하고 기대한 HTTP 상태 코드가 반환되는지 확인합니다.
